...
def main(data_path_head,data_path_tail,args):   
      ...
      ### ********** Evaluate features in combinations of 2 ********** ###
      feature__idxs = np.arange(len(eiva_feature_names[3:]))+3
      combinations_tpls = list(itertools.combinations(feature__idxs,args.combinations))
      print(combinations_tpls)

      for feature_comb in combinations_tpls:
            feature_names = list(np.array(eiva_feature_names)[[*feature_comb]])
            feature = np.array(feature_comb)
            ...
            if args.knn is True:
                  ...
                  for k_neighbors in 2**np.arange(9):
                        ...
                        classifier_name = "kNN_k{}_".format(k_neighbors)+"Feature{}={}".format(feature_comb,feature_names)
            ...
            if args.nb is True:
                  ...
                  classifier_name ="NB_"+"Feature{}={}_".format(feature_comb,feature_names)
                  print("\t"+classifier_name)
            ...
            if args.svm is True:
                  for C in (2.**np.arange(-5,16,10)).tolist():
                        ...
                        for tol in (10.**np.arange( -5, 0, 2 )).tolist():
                              ...
                              classifier_name = "LinSVM_C{}_tol{}_".format(C,tol)+"Feature{}={}_".format(feature_comb,feature_names)
                              print("\t"+classifier_name)
                              ...
                              for gamma in (2.**np.arange(-10,0,3)).tolist():
                                    ...
                                    classifier_name = "RbfSVM_C{}_tol{}_gamme{}".format(C,tol,gamma)+"Feature{}={}_".format(feature_comb,feature_names)
                                    print("\t"+classifier_name)
...
if __name__ == "__main__":
      ...
      parser = argparse.ArgumentParser(prog='EIVA R&D Project',
                                          ...
                                          learning algorithms to classify noisy samples based on the chosen number of combined features''')
